    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            if self.gesture_result:
                display_text(f"Detected: {self.gesture_result}")

                current_time = time.time()
                if current_time - last_message_time >= 2:
                    if self.gesture_result == "Open_Palm":
                        # display_text(f"Test send msg: {self.gesture_result}")
                    ...
                    last_message_time = current_time  # Update last message timestamp
